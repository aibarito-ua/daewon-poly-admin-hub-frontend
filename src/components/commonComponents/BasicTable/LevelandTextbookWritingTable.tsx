...
const TableBody = (props:{dataModel:{key:string, value:any, rowspan:number, print:boolean}[][], rowMergeKey: string[]}) => {
    ...
    // console.log('merge test =',gradeHaveCell)
    // console.log('merge keys =',gradesKey)
...
export default function LevelAndTextbookWritingTableComponent (props:{table:Table<any>, options?:{sortEventTargetHeaderKeys?: string[],mergeRowSpanKeys?: string[]}}) {
    // table row datas
        ...
        for (let rowDIndex = 0; rowDIndex < rowD.length; rowDIndex++) {
            ...
            if (check) {
                const pushCellData = {
                    ...
                    value: rowD[rowDIndex].column.id!=='grade' ? rowD[rowDIndex].getValue():'사용 안함',
                    rowspan:1,
                    print:true
                }
