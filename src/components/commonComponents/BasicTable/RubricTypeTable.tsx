    ...
    return (
        <thead className='text-xs uppercase'>
            {props.table.getHeaderGroups().map((headerGroup) => {
                return (
                    <tr key={headerGroup.id} className='bg-[#dae3f4]'>
                        {headerGroup.headers.map((header) => {
                            
                            return (
                                <th
                                    key={header.id}
                                    className='px-2 py-2 w-fit text-center'
                                    // style={{
                    ...
                    <tr key={'row-data-'+rIdx}>
                        {row.map((cell, cIdx) => {
                            if (cell.print === true) {
                                return (
                                    <td key={cell.key}
                                    className={` py-2 font-medium whitespace-nowrap border ${
                                        cell.key==='year'||cell.key==='semester'||cell.key==='grade'||cell.key==='number_of_units' ? 'text-center': 'px-2'
                                    }`}
...
export default function TableComponent (props:{table:Table<any>, options?:{sortEventTargetHeaderKeys?: string[],mergeRowSpanKeys?: string[]}}) {
    ...
    if (dataModel.length > 0) {
        return (
            <div className='flex flex-wrap w-full h-fit relative shadow-md rounded-md'>
